<template>
  <div>
    <v-container>
      <v-card>
        <v-card-title class="text-h3 justify-center font-weight-bold"
          >Personal Page</v-card-title
        >
        ...
        <v-row>
          <v-layout align-center justify-center>
            ...
            <v-tooltip left>
              <template v-slot:activator="{ on, attrs }">
                <v-btn rounded v-bind="attrs" v-on="on" @click="skill = true">
                  編集
      ...
      <v-dialog v-model="password" max-width="600" persistent>
        <v-card>
          ...
          <v-card-actions>
            <v-layout align-center justify-center>
              <v-spacer />
              <v-btn class="error" flat @click="password = false"
                >取り消し</v-btn
              >
              <v-spacer />
